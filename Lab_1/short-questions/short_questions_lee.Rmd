...
cereal <- cereal %>%
    mutate(sodium_g = (sodium_mg / 1000)) %>%
    mutate(
        Shelf = factor(Shelf),
...
Rescale2 <- function(value, col) {
    minn <- ifelse(value < min(cereal[col]), value, min(cereal[col]))
    maxx <- ifelse(value > max(cereal[col]), value, max(cereal[col]))
