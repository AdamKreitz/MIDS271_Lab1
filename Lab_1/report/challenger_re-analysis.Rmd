...
```{r, include=F}
challenger <- read.csv("../data/raw/challenger.csv")
```
...
We generated a linear model based on Temperature and then proceeded to run tests to check the linearity of the data. It is also important to note that for the linear model, we treated the response variable as the number of O.ring failures, rather than modeling it as a binomial. The results were clear from examining the pattern of residuals to see if they were randomly distributed. To do so, we generated a QQ plot (below) in order to check for normalized distribution of residuals.  

```{r qq plot, echo=F, fig.align="center", fig.height=4}
# Define a linear model
linear_model <- lm(O.ring ~ Temp, data = challenger)
# Predict probabilities
challenger$predicted <- predict(linear_model, type = "response")
...
We can see that the residuals are not normally distributed in the QQ plot. This tells us that a linear model is likely not a good fit for the data. We see that at a lower quantile, the residuals tend to be highly positive, and at higher quantiles the residuals also tend to be highly negative. In the middle of the data, it tends to oscillate positive and negative with smaller absolute residuals. Having these kinds of patterns in the QQ plot is not what you will see with a model that has normal residuals..

We also want to check for homoscedasticity in the residuals, or in other words that the residuals in our fitted linear model are random in nature.

In this case, the red line represents predicted values, and we can see definitively that the data does not have a strong linear relationship. We also see that at lower temperatures the residuals tend to be above the line, whereas at higher temperatures the residuals tend to be at a lower line, which violates the assumption of homoscedasticity. Based on these two things, we can confidently conclude that a linear model is not a good fit for this dataset.
...
BoxPlot(cereal, "sugar") + theme(legend.position = "none") | BoxPlot(cereal, "fat") + theme(legend.position = "none") | BoxPlot(cereal, "sodium")
```
