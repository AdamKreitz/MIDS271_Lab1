...
```{r scatter plots of Temp vs O.ring and Pressure vs O.ring, echo=F, fig.align="center", fig.height=3}
plot(challenger$Temp, challenger$O.ring,
    main = "O-ring failures by Temperature",
    xlab = "Temperature", ylab = "O.ring",
    pch = 16)
# | plot(challenger$Pressure, challenger$O.ring, pch = 16, xlab = "Pressure", ylab = "O.ring")
```

The next graph we created was a scatterplot of Pressure vs. O.Ring. This scatterplot does not make pressure seem to be a relevant factor, as we see both low pressure and high pressure have instances of O-ring failure, though it is not definitive because there are many points of overlap. It's interesting to see that at a Pressure of 100, there are no instances of O-ring failure, so there is the possibility that only extremes on either side could influence failure, but more inspection is needed.

```{r scatter plot of Pressure vs O.ring, echo=F, fig.align="center", fig.height=3}
plot(challenger$Pressure, challenger$O.ring,
    main = "O-ring failures by Pressure",
    xlab = "Pressure", ylab = "O.ring",
    pch = 16)
# challenger %>% ggplot(aes(y = factor(O.ring), x = Pressure)) +
#     geom_point() +
#     ylab("O.ring")
#     labs(title = "O-ring Failures by Pressure", color = "O-Ring Failures")
```
...
```{r heatmap of correlation matrix, echo=F, fig.align="center", fig.height=3}
hist(challenger$Temp, xlab = "Temperature", main = "Histogram of Temperature")
hist(challenger$Pressure, xlab = "Pressure", main = "Histogram of Pressure")
```
...
```{r confidence interval plot1, echo=F, fig.align="center", fig.height=3}
...
```{r confidence interval plot 2, echo=F, fig.align="center", fig.height=3}
...
```{r bootstrap confidence interval plot, echo=F, fig.align="center", fig.height=3}
